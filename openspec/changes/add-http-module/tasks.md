...
- [x] 6.2 Umbrella `http.h` — **not needed** (module headers include directly; `xpp/http.h` vs `xpp/http/*.h` is confusing)
- [ ] 6.3 Top-level `xpp::http::get/post/...` — **DEFERRED** (prototyped, rejected: default Client is unconfigured, and its lifetime must span the whole transfer in C++ — see design.md Q1)
- [x] 6.4 Write `http_convenience_test.cpp` (Layer 3) using `TestServer`: Client convenience methods round-trip each verb; each URL overload compiles and round-trips
- [ ] 6.5 Default Client / cold-start profile — **DEFERRED** with 6.3 (no top-level functions use a default Client)
