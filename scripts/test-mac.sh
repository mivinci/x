...
#
# Usage:
#   scripts/test-mac.sh -t <openssl|mbedtls> [-j <jobs>] [-B <build-dir>] (--asan|--tsan)
...
if [[ $ASAN -eq 1 ]]; then
  ...
  CMAKE_EXTRA_FLAGS="$CMAKE_EXTRA_FLAGS -DCMAKE_C_FLAGS='-fsanitize=thread -fno-omit-frame-pointer -g' -DCMAKE_CXX_FLAGS='-fsanitize=thread -fno-omit-frame-pointer -g'"
fi
