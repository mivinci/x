...
auto all(Promise<Ts>... promises)
  ...
  auto *node = _promise::allocate<_::AllVoidPromiseNode<N>>(nullptr, std::move(promises)...);
...
auto all(Promise<Ts>... promises)
  ...
  static_assert(sizeof...(Ts) > 0, "all() requires at least one promise");
  auto *node = _promise::allocate<_::AllTuplePromiseNode<Ts...>>(nullptr, std::move(promises)...);
...
template <class T, class... Rest> Promise<T> race(Promise<T> first, Promise<Rest>... rest) {
  ...
  auto            *node = _promise::allocate<_::RacePromiseNode<typename FixVoid<T>::Type, N>>(
