...
TEST(TcpConnTest, ConnectFailure) {
  ...
  auto result = TcpStream::connect("127.0.0.1:1").await();
...
TEST(TcpConnTest, ConnectAndSendRecv) {
  ...
  auto listener_r = TcpListener::bind(("127.0.0.1:" + std::to_string(port)).c_str()).await();
   ...
   * Sequential .await() calls avoid the all() combinator, whose
  ...
  auto connect_r = TcpStream::connect(("127.0.0.1:" + std::to_string(port)).c_str()).await();
  ...
  // Client send
  ssize_t sent = client_conn.write(msg, 5).await();
  EXPECT_EQ(sent, 5);

  // Server recv
  ssize_t recvd = server_conn.read(server_buf->data(), server_buf->size()).await();
  EXPECT_EQ(recvd, 5);
  EXPECT_EQ(std::string(server_buf->data(), static_cast<size_t>(recvd)), "hello");

  // Server echo back
  ssize_t echoed = server_conn.write(server_buf->data(), static_cast<size_t>(recvd)).await();
  EXPECT_EQ(echoed, 5);

  // Client recv echo
  ssize_t echoed_recvd = client_conn.read(client_buf->data(), client_buf->size()).await();
