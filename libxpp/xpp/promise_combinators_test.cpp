...
static xTimer schedule_resolve_int(xpp::PromiseResolver<int> &r, int value,
                                    uint64_t delay_ms) {
  struct Ctx { xpp::PromiseResolver<int> *r; int value; };
...
static xTimer schedule_resolve_str(xpp::PromiseResolver<std::string> &r,
                                    const char *val, uint64_t delay_ms) {
  struct Ctx { xpp::PromiseResolver<std::string> *r; std::string val; };
  auto *ctx = new Ctx{&r, val};
  return xTimerStart([](void *a) { auto *c = static_cast<Ctx *>(a);
    c->r->resolve(std::move(c->val)); delete c; }, ctx, nullptr, delay_ms, 0);
}

static xTimer schedule_resolve_void(xpp::PromiseResolver<void> &r,
                                     uint64_t delay_ms) {
  struct Ctx { xpp::PromiseResolver<void> *r; };
  auto *ctx = new Ctx{&r};
  return xTimerStart([](void *a) { auto *c = static_cast<Ctx *>(a);
...
TEST(AllTest, ImmediateHeterogeneous) {
  ...
  auto t = xpp::all(xpp::Promise<int>::resolve(42),
                    xpp::Promise<std::string>::resolve(std::string("hi"))).wait();
...
TEST(AllTest, ImmediateAllVoid) {
  ...
  xpp::all(xpp::Promise<void>::resolve(),
           xpp::Promise<void>::resolve(),
           xpp::Promise<void>::resolve()).wait();
  SUCCEED();
}
...
TEST(AllTest, ImmediateVoidAndValue) {
  ...
  auto t = xpp::all(xpp::Promise<void>::resolve(),
                    xpp::Promise<int>::resolve(7)).wait();
...
TEST(AllTest, ImmediateThreeTypes) {
  ...
  auto t = xpp::all(xpp::Promise<int>::resolve(1),
                    xpp::Promise<double>::resolve(2.5),
                    xpp::Promise<std::string>::resolve(std::string("three"))).wait();
...
TEST(AllTest, DeferredWithTimers) {
  ...
  xpp::all(xpp::Promise<void>::after(10),
...
TEST(AllTest, ThenChain) {
  ...
  int sum = xpp::all(xpp::Promise<int>::resolve(10),
                     xpp::Promise<int>::resolve(20))
...
TEST(RaceTest, ImmediateFirstWins) {
  ...
  int result = xpp::race(xpp::Promise<int>::resolve(1),
                         xpp::Promise<int>::resolve(2)).wait();
  EXPECT_EQ(result, 1);
}
...
TEST(RaceTest, ImmediateOneDeferred) {
  ...
  int result = xpp::race(xpp::Promise<int>::resolve(42),
                         xpp::Promise<void>::after(100).then([] { return 99; })).wait();
  EXPECT_EQ(result, 42);
}
...
TEST(RaceTest, FasterTimerWins) {
  ...
  int result = xpp::race(
      xpp::Promise<void>::after(50).then([] { return 1; }),
      xpp::Promise<void>::after(10).then([] { return 2; })).wait();
  EXPECT_EQ(result, 2);
}
...
TEST(RaceTest, VoidRace) {
  ...
  xpp::race(xpp::Promise<void>::after(10),
            xpp::Promise<void>::after(50)).wait();
  SUCCEED();
}
...
TEST(RaceTest, VoidRaceImmediate) {
  ...
  xpp::race(xpp::Promise<void>::resolve(),
            xpp::Promise<void>::after(50)).wait();
  SUCCEED();
}
...
TEST(RaceTest, TimerStoppedOnEarlyResolve) {
  ...
  xpp::race(xpp::Promise<int>::resolve(42),
            xpp::Promise<void>::after(10000).then([] { return 0; })).wait();
...
TEST(RaceTest, ThenChain) {
  ...
  int doubled = xpp::race(
      xpp::Promise<int>::resolve(21),
      xpp::Promise<void>::after(50).then([] { return 99; }))
      .then([](int x) { return x * 2; }).wait();
