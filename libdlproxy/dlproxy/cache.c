...
static void meta_save(struct dlp_clip *cl, const char *res_dir) {
  ...
  r.mode   = 0644;
  if (xFsReqSubmit(&r) != xErrno_Ok || !r.out_file) {
    free(buf);
    return;
  }
...
static bool meta_load(struct dlp_clip *cl, const char *res_dir) {
  ...
  if (memcmp(buf, META_MAGIC, META_MAGIC_LEN) != 0) {
    free(buf);
    return false;
  }
  ...
  bool all_done = true;
  for (uint32_t i = 0; i < cl->block_count; i++) {
    if (!cl->blocks[i] || !cl->blocks[i]->done) {
      all_done = false;
      break;
    }
  }
...
xErrno dlp_cache_write(dlp_cache_t c, const char *rid, const char *clip_id, uint64_t offset,
...
xErrno dlp_cache_read(dlp_cache_t c, const char *rid, const char *clip_id, uint64_t offset,
  ...
  if (!dlp_cache_is_ready(c, rid, clip_id, offset, len)) return xErrno_NotFound;
...
int dlp_cache_is_ready(dlp_cache_t c, const char *rid, const char *clip_id, uint64_t offset,
                       size_t len) {
