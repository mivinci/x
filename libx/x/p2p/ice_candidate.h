 ...
 */
XCAPI(int) xIceCandidateTypePref(xIceCandidateType type);
 ...
 */
XCAPI(void) xIceCandidateFoundation(xIceCandidate *cand, const struct sockaddr *stun_server);

/**
 * @brief Get the candidate type as a string (for SDP).
 */
XCAPI(const char *) xIceCandidateTypeStr(xIceCandidateType type);
 ...
 */
XCAPI(xErrno) xIceCandidateTypeFromStr(const char *str, xIceCandidateType *type);

/**
 * @brief Get the port from a sockaddr.
 */
XCAPI(uint16_t) xSockaddrPort(const struct sockaddr *addr);
 ...
 */
XCAPI(const char *) xSockaddrIP(const struct sockaddr *addr, char *buf, size_t len);
