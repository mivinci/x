...
TEST(BuiltinTimerPrecision, UnderDoneQueueFlood) {
  ...
  for (int i = 0; i < 1000; i++)
    xEventLoopPost(
      loop, [](void *arg) { static_cast<std::atomic<int> *>(arg)->fetch_add(1); }, &posted_count);
...
TEST(BuiltinTimerPrecision, JitterAnalysis) {
  ...
  double stddev_ns =
    actual_fires > 1 ? std::sqrt((sum_sq - sum * sum / actual_fires) / (actual_fires - 1)) : 0;

  printf("[timer-jitter] N=%d fires=%d min=%.0f ns max=%.0f ns mean=%.1f ns stddev=%.1f ns  "
         "(%.3f ms / %.3f ms)\n",
         N, actual_fires, min_jitter_ns, max_jitter_ns, mean_ns, stddev_ns, mean_ns / 1e6,
         stddev_ns / 1e6);
