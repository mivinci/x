...
TEST(BuiltinTimerConcurrent, CrossThreadStopStress) {
  ...
  for (int i = 0; i < N; i++)
    timers[i] = xTimerStart([](void *arg) { static_cast<std::atomic<int> *>(arg)->fetch_add(1); }, &fired, 2000, 0);
  ...
  for (int i = 0; i < N; i++) {
    ...
    xEventLoopPost(loop,
      [](void *arg) {
        ...
        p->second++;
        xTimerStart([](void *a) { static_cast<std::atomic<int> *>(a)->fetch_add(1); }, p->first, 0, 0);
        delete p;
      }, ctx);
  }
...
TEST(BuiltinTimerConcurrent, RepeatTimerUnderCrossThreadStop) {
  ...
  xTimer t = xTimerStart([](void *arg) { static_cast<std::atomic<int> *>(arg)->fetch_add(1); }, &count, 20, 20);

  {
    xTimer t = xTimerStart([](void *arg) { xEventLoopStop(static_cast<xEventLoop>(arg)); }, loop, 200, 0);
    xEventLoopRun(loop, X_RUN_DEFAULT);
    if (t) xTimerStop(t);
  }
  EXPECT_GE(count.load(), 1) << "repeat timer should have fired";

  std::thread stopper([t]() { xTimerStop(t); }); stopper.join();
  {
...
TEST(BuiltinTimerHybrid, StartTimerFromCallback) {
  ...
  xTimerStart(
    [](void *arg) {
      ...
      xTimerStart([](void *a) { static_cast<std::atomic<int> *>(a)->fetch_add(1); }, ctx->second, 10, 0);
      delete ctx;
    }, pair, 50, 0);
...
TEST(BuiltinTimerHybrid, CancelFromAnotherCallback) {
  ...
  struct Ctx { std::atomic<int> a_count, b_count; xTimer b_timer; };
  Ctx ctx{};
  ...
  ctx.b_timer = xTimerStart([](void *arg) { static_cast<Ctx *>(arg)->b_count.fetch_add(1); }, &ctx, 100, 0);
...
TEST(BuiltinTimerHybrid, InterleavedRepeatTimers) {
  ...
  xTimer slow_t = xTimerStart([](void *arg) { static_cast<std::atomic<int> *>(arg)->fetch_add(1); }, &slow, 30, 60);
