...
TEST(Md5, Empty) {
  ...
  ASSERT_EQ(xMd5(reinterpret_cast<const uint8_t *>(""), 0, digest), xErrno_Ok);
...
TEST(Md5, Hello) {
  ...
  uint8_t     digest[XCRYPTO_MD5_DIGEST_SIZE];
  ASSERT_EQ(xMd5(reinterpret_cast<const uint8_t *>(input), strlen(input), digest), xErrno_Ok);
...
TEST(Md5, LongTermCredential) {
  ...
  ASSERT_EQ(xMd5(reinterpret_cast<const uint8_t *>(input), strlen(input), digest2), xErrno_Ok);
  ...
  ASSERT_EQ(xMd5(reinterpret_cast<const uint8_t *>(input2), strlen(input2), digest3), xErrno_Ok);
...
TEST(Md5, NullArgs) {
  ...
  EXPECT_EQ(xMd5(reinterpret_cast<const uint8_t *>("x"), 1, NULL), xErrno_InvalidArg);
}
