...
TEST(Sha256, EmptyString) {
  ...
  ASSERT_EQ(xSha256(reinterpret_cast<const uint8_t *>(""), 0, digest), xErrno_Ok);
...
TEST(Sha256, Abc) {
  ...
  uint8_t     digest[XCRYPTO_SHA256_DIGEST_SIZE];
  ASSERT_EQ(xSha256(reinterpret_cast<const uint8_t *>(input), strlen(input), digest), xErrno_Ok);
  EXPECT_EQ(hex(digest, XCRYPTO_SHA256_DIGEST_SIZE),
...
TEST(Sha256, MillionAs) {
  ...
  uint8_t     digest[XCRYPTO_SHA256_DIGEST_SIZE];
  ASSERT_EQ(xSha256(reinterpret_cast<const uint8_t *>(input.data()), input.size(), digest),
            xErrno_Ok);
  EXPECT_EQ(hex(digest, XCRYPTO_SHA256_DIGEST_SIZE),
...
TEST(Sha256, StreamingMatchesOneShot) {
  ...
  ASSERT_EQ(xSha256(reinterpret_cast<const uint8_t *>(full), strlen(full), digest1), xErrno_Ok);

  /* Streaming */
  uint8_t    digest2[XCRYPTO_SHA256_DIGEST_SIZE];
  xSha256Ctx ctx;
  ASSERT_EQ(xSha256Init(&ctx), xErrno_Ok);
  ASSERT_EQ(xSha256Update(&ctx, reinterpret_cast<const uint8_t *>(parts[0]), strlen(parts[0])),
            xErrno_Ok);
  ASSERT_EQ(xSha256Update(&ctx, reinterpret_cast<const uint8_t *>(parts[1]), strlen(parts[1])),
            xErrno_Ok);
...
TEST(Sha256, NullArgs) {
  ...
  EXPECT_EQ(xSha256(reinterpret_cast<const uint8_t *>("x"), 1, NULL), xErrno_InvalidArg);
