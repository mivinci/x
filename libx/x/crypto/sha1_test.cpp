...
TEST(Sha1, EmptyString) {
  ...
  ASSERT_EQ(xSha1(reinterpret_cast<const uint8_t *>(""), 0, digest), xErrno_Ok);
...
TEST(Sha1, Abc) {
  ...
  uint8_t     digest[XCRYPTO_SHA1_DIGEST_SIZE];
  ASSERT_EQ(xSha1(reinterpret_cast<const uint8_t *>(input), strlen(input), digest), xErrno_Ok);
...
TEST(Sha1, MillionAs) {
  ...
  uint8_t     digest[XCRYPTO_SHA1_DIGEST_SIZE];
  ASSERT_EQ(xSha1(reinterpret_cast<const uint8_t *>(input.data()), input.size(), digest),
            xErrno_Ok);
...
TEST(Sha1, StreamingMatchesOneShot) {
  ...
  ASSERT_EQ(xSha1(reinterpret_cast<const uint8_t *>(full), strlen(full), digest1), xErrno_Ok);

  /* Streaming */
  uint8_t  digest2[XCRYPTO_SHA1_DIGEST_SIZE];
  xSha1Ctx ctx;
  ASSERT_EQ(xSha1Init(&ctx), xErrno_Ok);
  ASSERT_EQ(xSha1Update(&ctx, reinterpret_cast<const uint8_t *>(parts[0]), strlen(parts[0])),
            xErrno_Ok);
  ASSERT_EQ(xSha1Update(&ctx, reinterpret_cast<const uint8_t *>(parts[1]), strlen(parts[1])),
            xErrno_Ok);
...
TEST(Sha1, NullArgs) {
  ...
  EXPECT_EQ(xSha1(reinterpret_cast<const uint8_t *>("x"), 1, NULL), xErrno_InvalidArg);
