...
auto client_p = TcpConn::connect("127.0.0.1", 9090).then([](TcpConn c) {
    return c.send("hi", 2).then([c](ssize_t) mutable {
        char buf[64];
        return c.recv(buf, 64);
    });
});
client_p.wait();
```
...
3. **adapt() for one-shot ops** — `lookup_host()` uses `adapt<T, Adapter>()` — the adapter starts the async op in its constructor, cancels in its destructor, and the `AdapterPromiseNode` owns the adapter. `TcpConn::connect()` uses `async() + new` (self-deleting adapter) because libx's `xTcpConnect` has no cancel API.

4. **TLS is transparent** — Pass `Option<const TlsContext&> = none` to `TcpConn::connect()` to enable TLS. libx's `xTcpConnect` does the handshake; the resulting `TcpConn` transparently encrypts/decrypts. No separate `TlsConn` type.

5. **RAII everywhere** — `TcpConn`, `TcpListener`, `UdpSocket`, `Url`, `TlsContext` all close/free their underlying resources in destructors. Move-only.
...
```text
TCP                           UDP                    DNS                URL/TLS
├── TcpConn                   ├── UdpSocket          ├── lookup_host()  ├── Url (sync)
...
- [TCP](tcp.md) — `TcpConn` and `TcpListener`: Promise-based async TCP.
...
| TCP connect | `Promise<TcpConn>` (adapt) | `Future<Result<TcpStream>>` |
