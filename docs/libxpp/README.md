...
// Promise<T> — poll-based async, calling thread is the executor
// (wait() drives the event loop; C++20 coroutines optional)
...
- **Poll-based Promise** — no background thread pool. `wait()` drives the event loop on the calling thread; `co_await` is optional (C++20).
