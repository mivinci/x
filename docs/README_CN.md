...
我们选择无栈，理由有二。第一，C++20 用 `co_await`/`co_return` 标准化了无栈协程，给了我们一个编译器原生的、可移植的、可优化的代码生成路径。第二，无栈协程和零开销抽象更加契合——编译器可以跨越协程边界内联、可以死代码消除未使用的状态变量、可以用自定义分配器分配协程帧。相比之下，有栈方案每个协程都需要预分配栈空间（通常 2–8 KB），上万个协程的内存开销就很可观了，而且运行时无法感知协程内的变量生命周期，优化空间非常有限。
...
所有这一切都运行在 **libx** 之上，一个 C99 库，提供了事件循环、非阻塞 I/O、定时器，以及无锁 MPSC 队列等底层基础设施。libx 建基于同样的理念：给 C 开发者一个开箱即用的异步运行时——没有回调地狱，不需要手动管理 fd，只需要 `xTcpConnect()` 和一个 promise 式的回调。libxpp 是在此之上的 C++ 层，增加了类型安全、移动语义和协程的易用性——名字本身就是这个关系：底层 C 库叫 **libx**，上层 C++ 库自然就叫 **libxpp**。
...
最终形成的技术栈让 C 和 C++ 各自获得它们应得的异步体验：libx 给需要结构化并发的系统程序员，libxpp 给想写下 `co_await tcp_stream.read(buf)` 或 `promise.then([](auto v){...})` 的应用开发者——没有裸指针，没有泄漏的抽象，没有回调金字塔。
